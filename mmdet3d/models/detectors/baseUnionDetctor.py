...
class BaseUnionDetector(BaseDetector):
    """Base class for union detectors."""
    def __init__(self,
        ...
        if pts_semantic_head:
            # TODO: add semantic head
            pts_train_cfg = train_cfg.pts if train_cfg else None
            pts_semantic_head.update(train_cfg=pts_train_cfg)
            pts_test_cfg = test_cfg.pts if test_cfg else None
            pts_semantic_head.update(test_cfg=pts_test_cfg)
            self.pts_semantic_head = builder.build_head(pts_semantic_head)
    ...
    @property
    def with_pts_semantic(self):
        """bool: Whether the detector has a 3D box head."""
        return hasattr(self,
                       'pts_semantic_head') and self.pts_semantic_head is not None
    ...
    def extract_feat(self, points, img=None, img_metas=None):
        """Extract features from images and points."""
        img_feats = None #self.extract_img_feat(img, img_metas)
    ...
    def train_step(self, data, optimizer):
        ...
        if data['det']:
            sz = len(data['det']['img_metas'])
        else:
            sz = len(data['seg']['img_metas'])
        outputs = dict(
            loss=loss, log_vars=log_vars, num_samples=sz)

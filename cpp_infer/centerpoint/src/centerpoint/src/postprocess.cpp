...
PostProcessCuda::PostProcessCuda(cudaStream_t stream)
{
    ...
    checkCudaErrors(cudaMallocManaged((void **)&h_bboxes_res_, params_.bboxes_dim1 * params_.booxes_dim2 * (params_.booxes_dim3 + 1) * sizeof(float)));
    checkCudaErrors(cudaMallocManaged((void **)&h_bboxes_labels_, params_.bboxes_dim1 * params_.booxes_dim2 * sizeof(int)));
    checkCudaErrors(cudaMallocManaged((void **)&h_bboxes_num_, sizeof(int)));

    checkCudaErrors(cudaMallocManaged((void **)&h_seg_res_, params_.grid_x_size * params_.grid_y_size * sizeof(int)));
    checkCudaErrors(cudaMallocManaged((void **)&h_seg_scores_, params_.grid_x_size * params_.grid_y_size * sizeof(float)));
    return;
}

PostProcessCuda::~PostProcessCuda()
{
    checkCudaErrors(cudaFree(h_bboxes_res_));
    checkCudaErrors(cudaFree(h_bboxes_labels_));
    checkCudaErrors(cudaFree(h_bboxes_num_));
    checkCudaErrors(cudaFree(h_seg_res_));
    checkCudaErrors(cudaFree(h_seg_scores_));
    return;
}

int PostProcessCuda::doPostprocessCuda(const float *bboxes, const float *scores, const int *labels, const float *semantic, 
                                       std::vector<Bndbox> &res, SemanticBev &seg_bev)
{
    detPostprocess(bboxes, scores, labels, h_bboxes_res_, h_bboxes_labels_,h_bboxes_num_);
    segPostprocess(semantic, h_seg_res_, h_seg_scores_);
    checkCudaErrors(cudaDeviceSynchronize());
    // det 
    auto bbox_num = h_bboxes_num_[0];
    for (int i = 0; i < bbox_num; i++) {
        auto Bb = Bndbox(i, h_bboxes_res_[i * 8],
                    h_bboxes_res_[i * 8 + 1], h_bboxes_res_[i * 8 + 2], h_bboxes_res_[i * 8 + 3],
                    h_bboxes_res_[i * 8 + 4], h_bboxes_res_[i * 8 + 5], h_bboxes_res_[i * 8 + 6],
                    h_bboxes_labels_[i],
                    h_bboxes_res_[i * 8 + 7]);
        res.push_back(Bb);
    }
    // seg
    seg_bev = SemanticBev(params_.grid_x_size, params_.grid_y_size, h_seg_res_, h_seg_scores_);

    return 0;
}

int PostProcessCuda::detPostprocess(const float *bboxes, const float *scores, const int *labels, 
                                    float *bboxes_res, int *bboxes_label, int *bboxes_num) {
    checkCudaErrors(cudaMemsetAsync(bboxes_num, 0, sizeof(int), stream_));
    checkCudaErrors(postprocess_launch(bboxes, scores, labels, params_.score_thresh, bboxes_res, bboxes_label, bboxes_num, stream_));
    checkCudaErrors(cudaDeviceSynchronize());
    return 0;
}

int PostProcessCuda::segPostprocess(const float *semantic, int *seg_res, float *seg_scores) {
    checkCudaErrors(seg_postprocess_launch(semantic, params_.grid_x_size, params_.grid_y_size, 
                                           params_.seg_num_classes, seg_res, seg_scores, stream_));
    return 0;
}
